...
## بوابة المرحلة السابعة — النسخ والاستعادة

توثق سياسة الاحتفاظ وRPO/RTO في `backup-policy-ar.md`، ومسار الاستعادة الآمن في
`restore-runbook-ar.md`، وخطة التعافي في `business-continuity-ar.md`. لتشغيل
اختبار صناعي معزول يشمل التصدير والفحص وكلمة المرور الخاطئة والحزمة المعدلة
والاستعادة والتراجع والتدقيق:

```bash
env -u DATABASE_URL BACKUP_ACCEPTANCE_PORT=8391 pnpm backup:acceptance
```

التقرير القابل لإعادة التنفيذ موجود في
`backup-restore-acceptance-2026-09-17-ar.md`. لا يحل هذا الاختبار محل اعتماد
مسؤول البيانات أو تمرين Pilot؛ يجب تسجيل RPO/RTO الفعليين وتعيين مالك وبديل
لمفاتيح النسخ قبل G3.
...
## نقل المعرفة والتشغيل اليومي

قبل تسليم النظام لمسؤول تشغيل جديد، اتبع `onboarding-ar.md` ثم نفذ التمرين
الموثق في `knowledge-transfer-report-ar.md`. لا يُعد نقل المعرفة مكتملًا إلا
بعد أن ينفذ شخص ثانٍ التمرين دون مساعدة شفوية ويوقع على النتيجة.

المراجع التشغيلية موزعة كالتالي:

- `onboarding-ar.md`: إعداد نسخة جديدة وبناء Desktop وPostgreSQL وAndroid.
- `runbooks/`: إجراءات الفشل والاستعادة والمزامنة والمفاتيح والموارد.
- `adr/`: القرارات التي لا يجوز تغييرها ضمنيًا أثناء الصيانة.

## بوابة المرحلة السادسة: المزامنة

يشغّل الأمر التالي اختبار المزامنة الكامل على نسختين PGlite مؤقتتين، مع تنظيف
البيانات بعد الانتهاء:

```bash
pnpm sync:phase6
```

ويشغّل قياسًا اصطناعيًا صغيرًا لاختبار مسار المتجهات وترتيب الحزم وحجم النقل:

```bash
pnpm sync:performance
```

الاختبار الكامل يجب أن يثبت: التبادل المتقاطع، الحركات والدفعات، التعارض،
الرفض، الاستيراد المكرر، التصدير التفاضلي، التعديل، الحذف الناعم، تسويات
المعدات، وإعادة الإرسال دون مضاعفة الرصيد. لا تُستخدم بيانات إنتاج في هذه
الأوامر.

تفاصيل آلة الحالات وضمانات العقد ومصفوفة القبول وإجراءات الحوادث موجودة في:

- `sync-state-machine-ar.md`
- `sync-guarantees-ar.md`
- `sync-test-matrix-ar.md`
- `sync-performance-baseline-ar.md`
- `sync-incident-runbook-ar.md`
