...
function Rebuild-ElectronVariant {
  ...
  $zipPath = Join-Path $rel "windows\Damascus-Health-Directorate-v$version-Windows-$Variant.zip"
  ...
  $desiredAppName = "Damascus-Health-Directorate-$version"
  if ($appDir.Name -ne $desiredAppName) {
    $renamedAppDir = Join-Path $ex $desiredAppName
    Move-Item $appDir.FullName $renamedAppDir -Force
    $appDir = Get-Item $renamedAppDir
  }
  $legacyExe = Get-ChildItem $appDir.FullName -Filter '*.exe' -File |
    Where-Object { $_.Name -ne 'elevate.exe' } |
    Select-Object -First 1
  if ($null -ne $legacyExe -and $legacyExe.Name -ne 'Damascus Health Directorate.exe') {
    Rename-Item $legacyExe.FullName 'Damascus Health Directorate.exe' -Force
  }
  ...
  $pkgJson = '{"name":"damascus-health-directorate-desktop","productName":"Damascus Health Directorate","version":"4.0.3","main":"electron/main.cjs"}'
  ...
  $keySrc = Join-Path $root 'release-artifacts\v4.0.3\license-public-keys\windows.b64'
  if (Test-Path $keySrc) {
