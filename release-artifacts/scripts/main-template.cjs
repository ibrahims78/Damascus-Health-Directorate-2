...
const RELEASE_VERSION = (() => { try { return require('../package.json').version; } catch { return '5.0.6'; } })();
...
async function createWindow() {
  ...
  desktopWindow = new BrowserWindow({
    ...
    icon: path.join(WEB_ROOT, "app-icon.ico"),
    webPreferences: {
...
app.whenReady().then(async () => {
  try {
    ...
    await dialog.showMessageBox({
      type: "error",
      title: "Damascus Health Directorate",
...
app.setAboutPanelOptions({
  applicationName: "Damascus Health Directorate",
  applicationVersion: RELEASE_VERSION,
  version: RELEASE_VERSION,
  copyright: "Damascus Health Directorate",
});
