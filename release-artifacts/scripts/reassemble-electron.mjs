...
function buildVariant(variant, webSource, apiSource) {
  const archiveName = `Damascus-Health-Directorate-v${version}-Windows-${variant}.zip`;
  ...
  const appName = `Damascus-Health-Directorate-${version}`;
  ...
  try {
    ...
    const executableName = "Damascus Health Directorate.exe";
    const legacyExecutable = fs
      .readdirSync(appDir)
      .find((name) => name.toLowerCase().endsWith(".exe") && name.toLowerCase() !== "elevate.exe");
    if (legacyExecutable && legacyExecutable !== executableName) {
      fs.renameSync(path.join(appDir, legacyExecutable), path.join(appDir, executableName));
    }
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "damascus-health-directorate-desktop",
        productName: "Damascus Health Directorate",
        version,
